...
prices into the final quotation-check CSV, ready to be written into a new,
date-named tab in the skill's canonical Google Sheet (see step 7 in
SKILL.md — the Autosheet MCP agent reads this file's contents and writes
them into that tab; this script does not talk to Google APIs itself).
...
def main():
    ...
    if unmatched:
        print(f"Warning: {len(unmatched)} best sellers had no usable match "
              f"(no xlsx row, or no Shopify price) and were skipped:", file=__import__('sys').stderr)
        for t in unmatched[:15]:
            print(f"  - {t}", file=__import__('sys').stderr)
        if len(unmatched) > 15:
            print(f"  ... and {len(unmatched) - 15} more", file=__import__('sys').stderr)
